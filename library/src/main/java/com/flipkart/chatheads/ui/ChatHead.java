...
public class ChatHead<T extends Serializable> extends ImageView implements SpringListener {
    ...
    float hideFactor = 0.3f;
    private float goToWallVelocity = 0;
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            ...
            if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL) {
                ...
                if (xMinValue == xMaxValue) {
                    xMinValue = -getMeasuredWidth() * hideFactor;
                    xMaxValue = getResources().getDisplayMetrics().widthPixels - ((1f - hideFactor)
                            * getMeasuredWidth());
                }
                int[] iconPosition = new int[2];
                getLocationOnScreen(iconPosition);
                boolean endX = iconPosition[0] > getResources().getDisplayMetrics().widthPixels / 2;
                float nearestXWall = endX ? xMaxValue : xMinValue;
                if (goToWallVelocity == 0)
                    goToWallVelocity = pxFromDp(700);
                float velocity = iconPosition[0] > nearestXWall ? -goToWallVelocity : goToWallVelocity;
                int xVelocity = (int) velocity;
                int yVelocity;
